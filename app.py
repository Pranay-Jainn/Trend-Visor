...
item1 = html.Div(
    [
        html.P("Trend-Visor", className="start"),
...
item2 = html.Div(
    [
        html.Div(
            [
                html.Img(src="/assets/logo.png", id='logo' ,className='logo'),
...
def forecast(n, n_days, val):
    if n is None or n_days is None or val is None:
        return [""]  # Return an empty graph if inputs are missing

    try:
        fig = prediction(val, int(n_days) + 1)
        return [dcc.Graph(figure=fig)]
    except Exception as e:
        return [html.Div(f"Error: {str(e)}")]  # Display the error message
